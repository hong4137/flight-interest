...
상시 서버가 없어 웹훅을 못 받으므로 **10분마다 확인**한다. GitHub 스케줄러는 부하가
몰리면 밀리므로 실제 응답까지 **10~30분**으로 보는 게 맞다. 그래서 버튼 대신 텍스트
명령을 쓴다 — 눌러도 몇 분간 무반응이면 고장난 것으로 느껴진다.

**조회만 필요하면 대시보드가 즉시 답을 준다.** 감시 도시, 날짜 창, 목표가, 현재 최저가가
모두 거기 있다. 명령은 값을 *바꿀* 때 쓰는 것이다.
